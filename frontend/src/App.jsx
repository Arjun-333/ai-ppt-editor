...
export default function App() {
  ...
  async function handleApplyInstruction(instr) {
    ...
    try {
      ...
      // update structure returned (plan_executed contains edits) — re-fetch would be more robust
  ...
  async function handleApplyElementEdit(elementId, newText) {
    ...
    setProcessing(true);
    try {
      // craft a minimal plan: replace single element
      ...
      // Our backend expects natural instructions, but we can call /edit with instruction containing a simple "apply this plan" -- to keep it simple, we'll call edit endpoint sending the same plan as instruction but you'd wire a dedicated endpoint
      // Simpler: call editFile with a special instruction string the backend recognizes (you can expand backend later)
  ...
  return (
    <div className="min-h-screen p-8 bg-black text-gray-100">
            ...
            <div className="card p-6 rounded-2xl">
              ...
              <div className="mt-3">
                {downloadUrl && (
                  <a
                    href={downloadUrl}
                    className="btn-brand px-4 py-2 rounded inline-block"
                  >
                    Download Edited PPT
                  </a>
                )}
      ...
      {processing && (
        <div className="fixed right-6 bottom-6 bg-black/70 p-4 rounded shadow-lg">
          Processing…
        </div>
      )}
    </div>
